...
if template_choice:
    response = requests.get(f"http://backend:8000/template/{template_choice}")
    ...
    if st.button("Save Template"):
        response = requests.post(f"http://backend:8000/template/{template_choice}", json={"content": new_content})
    ...
    if st.button("Send Email"):
        response = requests.post("http://backend:8000/send-email", json={
